...
# Usage: ./setup-controller.sh [controller_ip] [compute_ip]

set -e

# Default IPs (can be overridden by command line arguments)
...
if [[ "$CONTROLLER_IP" == "<controller_ip>" || "$COMPUTE_IP" == "<compute_ip>" ]]; then
    ...
    echo "Usage: $0 <controller_ip> <compute_ip>"
    echo "Example: $0 192.168.0.5 192.168.0.6"
    exit 1
fi
